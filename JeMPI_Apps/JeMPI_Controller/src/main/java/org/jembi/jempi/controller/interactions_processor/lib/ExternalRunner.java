package org.jembi.jempi.controller.interactions_processor.lib;
...
public final class ExternalRunner {
    ...
    public static void printSaved(final String reportFilePath, final Float matchThreshold, final Float windowLowerBound, final Float windowUpperBound) throws ExecutionException, InterruptedException {
        sleep(2000);
        HashMap<String, FieldEqualityPairMatchMatrix> resultMatrix = MuAccesor.getKafkaMUUpdater(GlobalConstants.DEFAULT_LINKER_GLOBAL_STORE_NAME,  AppConfig.KAFKA_BOOTSTRAP_SERVERS).getValue();
