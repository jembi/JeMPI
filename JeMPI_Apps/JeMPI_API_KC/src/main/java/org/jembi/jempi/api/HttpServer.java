                           ...
                           () -> {
                              final ExceptionHandler handler = ExceptionHandler.newBuilder()
                                 .match(Exception.class, x -> {
                                    ...
                                    // x.printStackTrace();
                                    return complete(StatusCodes.BAD_REQUEST, "BOOM!");
                                 }).build();

                              return handleExceptions(handler, () -> {
                                 LOGGER.info("Completing successfully, returning user: {}", user);
                                 return complete(StatusCodes.OK, user, Jackson.<User>marshaller());
