  ...
  def generate(config: Config): Any = {
    ...
    def createGenerateFunctions(): String = {
      config.demographicFields
        ...
        .map(f =>
          s"""   public static final Supplier<String> GENERATE_${Utils
              .camelCaseToSnakeCase(Utils.snakeCaseToCamelCase(f.fieldName))
              .toUpperCase} = ${f.source.get.generate.get.func};""".stripMargin
        )
      ...
      def applyFields(): String = {
        ...
        def applyFunction(f: DemographicField): String = {
          if (f.source.isDefined && f.source.get.generate.isDefined) {
            s"""GENERATE_${Utils
                .camelCaseToSnakeCase(Utils.snakeCaseToCamelCase(f.fieldName))
                .toUpperCase()}.get()""".stripMargin
          } else {
            s"""interaction.demographicData().${Utils.snakeCaseToCamelCase(
                f.fieldName
              )}""".stripMargin
          }
        }
        ...
        config.demographicFields
          .map(f => s"""${" " * 55}${applyFunction(f)},""".stripMargin)
          .mkString(sys.props("line.separator"))
    ...
    writer.println()
    writer.println(s"""import org.jembi.jempi.libmpi.LibMPI;
    ...
    writer.println()
    config.demographicFields
      .filter(field => field.isList.isDefined && field.isList.get)
      .foreach(field => {
