...
public final class Routes {
      ...
      // private static Route getAuthToken(
      // final ActorSystem<Void> actorSystem,
      // final ActorRef<BackEnd.Event> backEnd) {
      // return entity(Jackson.unmarshaller(ApiModels.ApiAuthToken.class), requestData
      // -> {
      // return onComplete(
      // Ask.getAuthToken(actorSystem, backEnd, requestData),
      // result -> {
      // if (!result.isSuccess()) {
      // final var e = result.failed().get();
      // LOGGER.error(e.getLocalizedMessage(), e);
      ...
      Route secureProxyRoute(Function<String, Route> proxyRouteFunction, SecretKey secretKey) {
            return entity(Unmarshaller.entityToString(), body -> {
                  try {
                        String decryptedBody = CryptoUtils.decrypt(body, secretKey);
                        Route proxyRoute = proxyRouteFunction.apply(decryptedBody);

                        return onSuccess(proxyRoute, response -> {
                              try {
                                    String encryptedResponse = CryptoUtils.encrypt(response.toString(), secretKey);
                                    return complete(HttpEntity.create(encryptedResponse));
                              } catch (Exception e) {
                                    return complete(StatusCodes.INTERNAL_SERVER_ERROR, "Encryption error");
                              }
                        });
                  } catch (Exception e) {
                        return complete(StatusCodes.INTERNAL_SERVER_ERROR, "Decryption error");
                  }
            });
      }
      ...
      public static Route createCoreAPIRoutes(
            ...
            return concat(post(() -> concat(
                        ...
                        get(() -> concat(
                                    ...
                                    // path(GlobalConstants.SEGMENT_PROXY_GET_AUTH_TOKEN,
                                    // () -> Routes.getAuthToken(actorSystem, backEnd)),
