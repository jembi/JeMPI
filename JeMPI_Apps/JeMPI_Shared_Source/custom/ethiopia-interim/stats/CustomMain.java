...
import org.jembi.jempi.libmpi.models.MpiExpandedGoldenRecord;
...
public final class CustomMain {
    ...
    private void updateStatsDataSet(MpiExpandedGoldenRecord goldenRecord) {
        final String goldenRecordAuxId = goldenRecord.entity().auxId();
        final String goldenRecordNumber = goldenRecordAuxId.substring(0, 12);

        final var entry = dataSet.get(goldenRecordNumber);
        final List<String> list = new ArrayList<>();
        goldenRecord.documentList().forEach(doc -> list.add(doc.entity().auxId()));
    ...
    private void displayGoldenRecordDocuments(final PrintWriter writer, final MpiExpandedGoldenRecord goldenRecord) {
        final var rot = goldenRecord.entity();
        if (writer != null) {
            writer.printf("GoldenRecord,%s,%s,%s,%s,%s,%s,%s%n",
                          rot.uid(), rot.auxId(),
                          rot.nameGiven(), rot.nameFather(), rot.gender(),
                          rot.dob(),
                          rot.phoneNumber());
            goldenRecord.documentList().forEach(doc -> {
                final var entity = doc.entity();
                writer.format(Locale.ENGLISH,
                              "document,%s,%s,%s,%s,%s,%s,%s,%f%n",
                              entity.uid(), entity.auxId(),
                              entity.nameGiven(), entity.nameFather(), entity.gender(),
                              entity.dob(),
                              entity.phoneNumber(),
                              doc.score());
            });
        ...
        System.out.printf("GoldenRecord,%s,%s,%s,%s,%s,%s,%s%n",
                          rot.uid(), rot.auxId(),
                          rot.nameGiven(), rot.nameFather(), rot.gender(),
                          rot.dob(),
                          rot.phoneNumber());
    ...
    private void processSubList(final PrintWriter writer, final int fromIdx, final int toIdx, final List<String> ids) throws IOException {
        ...
        goldenRecordDocuments.goldenRecords.forEach(rec -> {
            displayGoldenRecordDocuments(writer, rec);
        });
    }
