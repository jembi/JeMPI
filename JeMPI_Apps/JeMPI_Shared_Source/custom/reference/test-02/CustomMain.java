...
public final class CustomMain {
   ...
   private static final String URL = String.format("http://%s:%d", AppConfig.LINKER_SERVER_HOST,
                                                   AppConfig.LINKER_SERVER_PORT);
   ...
   private void linkEntity(final String stan, final CustomEntity entity) {
      ...
      try {
         var body = RequestBody.create(
               OBJECT_MAPPER.writeValueAsString(new LinkEntitySyncBody(stan, null, 0.65F, entity)),
   ...
   private void openCsvReadCSV(final String filename) {
      try (CSVReader reader = new CSVReaderBuilder(new FileReader(filename))
         ...
         while ((line = reader.readNext()) != null) {
            ...
            final var entity = new CustomEntity(null,
                                                null,
                                                line[0],
