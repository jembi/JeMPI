...
public class HttpServer extends HttpSessionAwareDirectives<UserSession> {
   ...
   private Route routeExpandedPatients(final ActorSystem<Void> actorSystem,
                                       final ActorRef<BackEnd.Event> backEnd) {
      return parameter("uidList",
                       items -> {
                          final var uidList = Stream.of(items.split(",")).map(String::trim).toList();
   ...
   private Route routeFindGoldenRecordByUid(final ActorSystem<Void> actorSystem, final ActorRef<BackEnd.Event> backEnd,
                                            final String uid) {
      return requiredSession(refreshable, sessionTransport, session ->
   ...
   private Route routeFindPatientRecordByUid(final ActorSystem<Void> actorSystem, final ActorRef<BackEnd.Event> backEnd,
                                             final String uid) {
      return requiredSession(refreshable, sessionTransport, session ->
   ...
   private Route routeLoginWithKeycloakRequest(final ActorSystem<Void> actorSystem, final ActorRef<BackEnd.Event> backEnd,
   ...
   private Route routeSimpleSearch(final ActorSystem<Void> actorSystem, final ActorRef<BackEnd.Event> backEnd,
   ...
   private Route routeCustomSearch(final ActorSystem<Void> actorSystem, final ActorRef<BackEnd.Event> backEnd,
                                   final RecordType recordType) {
      return requiredSession(refreshable, sessionTransport, session -> {
                                                  ...
                                                  () -> pathPrefix(
                                                        ...
                                                        () -> concat(
                                                              post(() -> concat(
                                                                    ...
                                                                    path("authenticate",
                                                                         () -> routeLoginWithKeycloakRequest(actorSystem, backEnd,
                                                                                                             checkHeader)),
                                                              ...
                                                              patch(() -> concat(
                                                                    path(segment("golden-record").slash(
                                                                         ...
                                                                         (uid) -> routeUpdateGoldenRecord(actorSystem, backEnd,
                                                                                                          uid)),
                                                                    path("Unlink",
                                                              ...
                                                              get(() -> concat(
                                                                    ...
                                                                    path("GoldenIdListByPredicate",
                                                                         () -> routeGoldenIdListByPredicate(actorSystem,
                                                                                                            backEnd)),
                                                                    path("GoldenRecordDocuments",
                                                                         () -> routeGoldenRecordDocuments(actorSystem, backEnd)),
                                                                    path("GoldenRecordDocumentList",
                                                                         () -> routeGoldenRecordDocumentList(actorSystem,
                                                                                                             backEnd)),
                                                                    path("ExpandedPatients",
                                                                         () -> routeExpandedPatients(actorSystem, backEnd)),
                                                                    path("MatchesForReview",
                                                                         () -> routeMatchesForReviewList(actorSystem, backEnd)),
                                                                    path(segment("patient-record").slash(
                                                                               segment(Pattern.compile("^[A-z0-9]+$"))),
                                                                         (uid) -> routeFindPatientRecordByUid(actorSystem,
                                                                                                              backEnd, uid)),
                                                                    path(segment("golden-record").slash(
                                                                               segment(Pattern.compile("^[A-z0-9]+$"))),
                                                                         (uid) -> routeFindGoldenRecordByUid(actorSystem, backEnd,
                                                                                                             uid)),
                                                                    path("Candidates",
