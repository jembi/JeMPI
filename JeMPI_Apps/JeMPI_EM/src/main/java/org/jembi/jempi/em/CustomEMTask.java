...
class CustomEMTask {
   ...
   private static Deserializer<InteractionEnvelop> patientJsonValueDeserializer() {
      return new JsonPojoDeserializer<>(InteractionEnvelop.class);
   }
   ...
   private void sendToKafka(
         final double[] mHat,
         final double[] uHat) throws InterruptedException {
      final CustomMU rec = new CustomMU(mHat, uHat);
      LOGGER.info("{}", rec);
      try {
         final var myProducer = new MyKafkaProducer<String, CustomMU>(AppConfig.KAFKA_BOOTSTRAP_SERVERS,
   ...
   private int[] setRowLevels(
      ...
      for (int i = 0; i < N_LINK_FIELDS; i++) {
         ...
         if (l == null || r == null) {
            ...
            row[i] = similarityScore.apply(l, r) >= 0.92
                  ? LEVELS - 1
                  : 0;
         }
      }
   ...
   private ArrayList<int[]> getGammaMatrix(
         final MyKafkaConsumerByPartition<String, InteractionEnvelop> consumer,
         final long nRecords) {
      ...
      while (busy && count[0] < nRecords) {
         ...
         if (records.isEmpty()) {
            ...
            records.forEach(r -> {
               if (r.value().contentType() == InteractionEnvelop.ContentType.BATCH_INTERACTION && count[0] < nRecords) {
                  count[0] += 1;
                  final var v = r.value();
      ...
      for (int j = 0; j < nRecords; j++) {
         ...
         for (int i = 0; i < nFields; i++) {
            ...
            if (gamma != MISSING_ELEMENT_INT) {
               ...
               numerator *= (gamma == (LEVELS - 1)
                                   ? m
                                   : (1.0 - m));
               denominator *= (gamma == (LEVELS - 1)
                                     ? u
                                     : (1.0 - u));
            }
         }
      ...
      for (int j = 0; j < nRecords; j++) {
         ...
         for (int i = 0; i < N_LINK_FIELDS; i++) {
            ...
            if (gamma != MISSING_ELEMENT_INT) {
               final var m = mHat[i];
               final var u = uHat[i];
               mProduct *= (gamma == (LEVELS - 1)
                                  ? m
                                  : 1.0 - m);
               uProduct *= (gamma == (LEVELS - 1)
                                  ? u
                                  : 1.0 - u);
            }
         }
