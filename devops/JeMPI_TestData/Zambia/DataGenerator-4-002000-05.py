...
def generate_dataset():
    ...
    csv_file_name = "results/dataset-4-002000-05-50.csv"
    ...
    for i in range(0, number_of_patients):
        ...
        for j in range(0, len(clinical_data)):
            ...
            data.append([rec_num, given_name, gender, dob, city, national_id, facility, patient_id, c_data ])
        k = k + 1
        if k % 1000 == 0:
            print(k)
    ...
    #                                  'city',
    #                                  'src_id_facility', 'src_id_patient', 'clinical_data'])
    df = pd.DataFrame(data, columns=['rec_num', 'given_name', 'gender', 'dob',
                                     'city', 'national_id',
                                     'src_id_facility', 'src_id_patient', 'clinical_data'])
    df['corrupted'] = False
